...
public class CardServiceImp implements CardService{
    ...
    @Override
    public List<ResponseCard> getCards(String language, String word, String example, String translateLanguage, String translation, String groups) {
        List<Card> cardList = repository.findAll();
        Predicate<Card> predicateByLanguage = (language == null || language.isEmpty())
          ? card -> true
                :card -> card.getLanguage().equalsIgnoreCase(language);

        Predicate<Card> predicateByWord = (word == null || word.isEmpty())
                ? card -> true
                :card -> card.getWord().toLowerCase().contains(word.toLowerCase());
        ...
        Predicate<Card> predicateByExample = (example == null || example.isEmpty())
                ? card -> true
                :card -> card.getExample().toLowerCase().contains(example.toLowerCase());
        ...
        Predicate<Card> allCondition = predicateByLanguage.and(predicateByWord)
